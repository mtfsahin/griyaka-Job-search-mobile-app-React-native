...
import { View, Text, FlatList, ActivityIndicator, AsyncStorage, Alert, TextInput, TouchableOpacity } from 'react-native';
  ...
  updateBrochures = async () => {
    ...
    .then(resp => {
        if (resp.status == 200) {
          ...
          Alert.alert("Başarılı", resp.data.message);
          this.fetchBrochuresData();
      } else if (resp.status == 203) {
        this.setState({ isLoading: false });
        console.log("in 203" , resp.data.message)
        Alert.alert("Hata", resp.data.message);
      }
    }).catch(err => {
        this.setState({ isLoading: false });
