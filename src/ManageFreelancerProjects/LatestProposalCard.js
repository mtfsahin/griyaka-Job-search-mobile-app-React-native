  ...
  downloadFile = async()=> {
    try {
      const granted_permission = await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE ,
        {
          title: "Depolama İzni",
          message: "Dosyayı indirmek için uygulamanın belleğe erişmesi gerekiyor."
        });
      if (granted_permission === PermissionsAndroid.RESULTS.GRANTED) {
        this.FileDownload();
      } else {
        Alert.alert('İzin reddedildi!', 'Dosyayı indirmek için depolama izni vermeniz gerekiyor');
      }
    } catch (err) {
      console.warn(err);
    } 
